...
for file in os.listdir(os.getcwd()):
    if file.endswith('.derep1.fa'):
        ...
        os.system(f"kraken2 --db /mnt/g/MU_WW/K2_viral {file} --report {samp_name}.derep1.allvk2report > {samp_name}.derep1.allv.k2")
        with open(f"{samp_name}.derep1.allv.minhitgroup25.k2", "r") as in_file:
            for line in in_file:
                if line.startswith("C"):
                    split_line = line.strip("\n\r").split("\t")
                    matched_ids_25[split_line[1]] = split_line[2]
        with open(f"{samp_name}.derep1.allv.k2", "r") as in_file:
            for line in in_file:
                if line.startswith("C"):
                    split_line = line.strip("\n\r").split("\t")
        ...
        with open(file, "r") as fa_in:
            with open(f"{samp_name}.minhitgroup25.k2_seq_matched_tax.txt", "w") as out_file:
                ...
                for line in fa_in:
                    if line.startswith(">"):
                        if seq_id in matched_ids_25 and sequence:
                            out_file.write(f"{seq_id}\t{tax_ids[matched_ids_25[seq_id]]}\t{sequence}\n")
                        seq_id = line.strip("\n\r")[1:]
                        sequence = ""
                    else:
                        sequence += line.strip("\n\r")
        ...
        with open(file, "r") as fa_in:
            with open(f"{samp_name}.k2_seq_matched_tax.txt", "w") as out_file:
                seq_id = ""
                sequence = ""
                for line in fa_in:
                    if line.startswith(">"):
