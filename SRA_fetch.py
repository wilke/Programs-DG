...
for line in args.file:
    ...
    if os.path.isfile(SRA_ID+'.collapsed.fa.gz') or os.path.isfile(SRA_ID+'.collapsed.fa'):
        ...
        os.system('fastq-dump ' + SRA_ID + ' --split-3')
        time.sleep(5)
        
        if os.path.isfile(SRA_ID+'_1.fastq') and os.path.isfile(SRA_ID+'_2.fastq'):
            print('paired reads')
            # os.system(f"bash /mnt/g/MU_WW/BBTools/bbmap/repair.sh in={SRA_ID}_1.fastq in2={SRA_ID}_2.fastq out={SRA_ID}_1.rep.fastq out2={SRA_ID}_2.rep.fastq outs={SRA_ID}_sing.rep.fastq")
            os.system(f"bash /mnt/g/MU_WW/BBTools/bbmap/bbmerge.sh qtrim=t in1={SRA_ID}_1.fastq in2={SRA_ID}_2.fastq  out={SRA_ID}.merge.fq outu1={SRA_ID}.un1.fq outu2={SRA_ID}.un2.fastq")
            os.system(f"cat {SRA_ID}.merge.fq {SRA_ID}.un1.fq {SRA_ID}.un2.fastq > {SRA_ID}.all.fq")
            if os.path.isfile(SRA_ID+'.fastq'):
                os.system(f"cat {SRA_ID}.fastq >> {SRA_ID}.all.fq")
            os.system(f"/mnt/g/MU_WW/vsearch/bin/vsearch --derep_fulllength {SRA_ID}.all.fq --output {SRA_ID}.collapsed.fa --sizeout --minuniquesize 1")
            

        elif os.path.isfile(SRA_ID+'.fastq'):
            print('singleton reads')
            os.system(f"/mnt/g/MU_WW/vsearch/bin/vsearch --derep_fulllength {SRA_ID}.fastq --output {SRA_ID}.collapsed.fa --sizeout --minuniquesize 1")
            
        elif os.path.isfile(SRA_ID+'_1.fastq'):
            print('singleton reads')
            os.system(f"/mnt/g/MU_WW/vsearch/bin/vsearch --derep_fulllength {SRA_ID}_1.fastq --output {SRA_ID}.collapsed.fa --sizeout --minuniquesize 1")
